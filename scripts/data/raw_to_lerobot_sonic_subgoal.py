...
GOAL_VIDEO_KEY = "observation.images.goal"
...
def episode_complete(ep_index: int, work_dir: Path, chunks_size: int, done_eps: set[int]) -> bool:
    ...
    ego_ok  = (work_dir / "videos" / f"chunk-{chunk_id:03d}" / EGO_VIDEO_KEY / f"episode_{ep_index:06d}.mp4").exists()
    goal_ok = (work_dir / "videos" / f"chunk-{chunk_id:03d}" / GOAL_VIDEO_KEY / f"episode_{ep_index:06d}.mp4").exists()
    stats_ok = ep_index in done_eps
    return data_ok and ego_ok and goal_ok and stats_ok
...
def load_subgoal_frames(episode_dir: Path) -> List[Tuple[int, Path]]:
    """Return [(frame_idx, path), ...] sorted by frame_idx for color_subgoal/."""
    subgoal_dir = episode_dir / "color_subgoal"
    if not subgoal_dir.is_dir():
        return []
    out: List[Tuple[int, Path]] = []
    for p in subgoal_dir.iterdir():
        if not p.is_file():
            continue
...
def assign_goal_paths(num_frames: int, subgoals: List[Tuple[int, Path]], rgb_paths: List[Path]) -> List[Path]:
    """For each frame i, return the path of the smallest subgoal idx >= i;
    frames past the last subgoal idx fall back to the last valid rgb frame."""
    if not subgoals:
        raise ValueError("No subgoal frames found in color_subgoal/")
    last_rgb = next((p for p in reversed(rgb_paths) if p is not None), None)
    result: List[Path] = []
    j = 0
    for i in range(num_frames):
        while j < len(subgoals) and subgoals[j][0] < i:
            j += 1
        if j < len(subgoals):
            result.append(subgoals[j][1])
        else:
            result.append(last_rgb)
    return result
...
class HE2LeRobotConverter:
    ...
    def make_one_episode(
        ...
        try:
            ...
            vid_chunk_dir = out_base.parent / "videos" / f"chunk-{episode_index // chunks_size:03d}" / EGO_VIDEO_KEY
            vid_chunk_dir.mkdir(parents=True, exist_ok=True)
            vid_path = vid_chunk_dir / f"episode_{episode_index:06d}.mp4"

            goal_chunk_dir = out_base.parent / "videos" / f"chunk-{episode_index // chunks_size:03d}" / GOAL_VIDEO_KEY
            goal_chunk_dir.mkdir(parents=True, exist_ok=True)
            goal_path = goal_chunk_dir / f"episode_{episode_index:06d}.mp4"
            ...
            def safe_path(episode_dir, f, key):
                ...
                if not p:
                    return None
                resolved = (episode_dir / p).resolve()
            ...
            if rgb_paths and rgb_paths[0] is None and len(rgb_paths) > 1 and rgb_paths[1] is not None:
                rgb_paths[0] = rgb_paths[1]

            subgoal_frames = load_subgoal_frames(episode_dir)
            goal_paths = assign_goal_paths(len(data_list), subgoal_frames, rgb_paths)
            ...
            goal_tmp    = tmp_dir / "goal.mp4"

            ds = Dataset.from_list(rows, features=self.features)
            ds.to_parquet(str(parquet_tmp))

            def frames_from(paths, label):
                for i, p in enumerate(paths):
                    if p is None:
                        raise ValueError(f"Frame {i} ({label}) in episode {episode_index} has no image path")
                    yield iio.imread(p)

            iio.imwrite(video_tmp, list(frames_from(rgb_paths,  "rgb")),  fps=FPS, codec="libx264")
            iio.imwrite(goal_tmp,  list(frames_from(goal_paths, "goal")), fps=FPS, codec="libx264")

            os.replace(parquet_tmp, parquet_path)
            os.replace(video_tmp, vid_path)
            os.replace(goal_tmp, goal_path)
    ...
    def write_meta(self, out_dir: Path):
        ...
        for ti, meta in self.tasks_meta.items():
            task_rows.append(
                {
                    "task_index": ti,
                    "task": meta.get("description") or meta.get("name", f"task_{ti:04d}"),
