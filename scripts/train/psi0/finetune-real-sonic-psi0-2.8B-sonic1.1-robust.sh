...
#
# Usage: bash scripts/train/psi0/finetune-real-sonic-psi0-2.8B-sonic1.1-robust.sh <task> [exp]

export OMP_NUM_THREADS=32
export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}

source "${PSI_VENV:-$([ -d /workspace/.venv-psi ] && echo /workspace/.venv-psi || echo .venv-psi)}/bin/activate"
DATA_ROOT=${DATA_ROOT:-.data/pnp_multi_object_with_wm}
TRAIN_IDS=${TRAIN_IDS:-pnp_multi_object_train/g1}
VAL_IDS=${VAL_IDS:-pnp_multi_object_val/g1}
...
# Default EMPTY here: no teleop5 task matches the zedmini recipe's "trash=4".
...
--data.transform.repack.dataset-name=pnpmultiobject \
...
echo "Running: torchrun --nproc_per_node=$NPROC_PER_NODE --master_port=${MAIN_PORT} scripts/train.py \\"
printf '  %s \\\n' ${args} | sed '$ s/ \\$//'
