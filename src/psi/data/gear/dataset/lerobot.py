...
from psi.data.gear.video_utils import (
    get_all_frames,
    get_frames_by_timestamps,
)
from psi.data.gear.schema import (
    DatasetMetadata,
...
from psi.data.gear.transform import ComposedModalityTransform
...
METADATA_DIR = Path(importlib.import_module("psi.data.gear").__file__).parent / "metadata"  # type: ignore
STEP_FILTER_FILENAME = "meta/step_filter.jsonl"
LEROBOT_RELATIVE_STATS_FILE_NAME = "meta/relative_stats_gear.json"
LEROBOT_RELATIVE_HORIZON_STATS_FILE_NAME = "meta/relative_horizon_stats_gear.json"
...
def calculate_dataset_statistics(
    ...
    for le_modality in features:
        logger.debug(f"Computing statistics for {le_modality}...")
        np_data = np.vstack(
...
class ExtendedModalityConfig(ModalityConfig):
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        action_steps_per_chunk: int = 32,
        num_latent_per_block: int = 2,
        ...
        Args:
            ...
            relative_action (bool): Whether to use relative action stats for normalization. If True, will load or calculate
                relative action stats from relative_stats_gear.json. If the file doesn't exist, stats will be calculated.
            ...
            relative_action_per_horizon (bool): Whether to use per-horizon relative action stats. If True, will load or calculate
                separate stats for each action horizon index from relative_horizon_stats_gear.json.
            allow_padding (bool): End-of-episode padding (psi0_simple-only). Set by the parent
                ShardedLeRobotMixtureDataset after construction; default off so standalone use of this class is unchanged.
        """
        # first check if the path directory exists
        if not Path(dataset_path).exists():
            raise FileNotFoundError(f"Dataset path {dataset_path} does not exist")

        logger.debug(f"Initializing {self.__class__.__name__} with path: {dataset_path}")
        ...
        self.action_steps_per_chunk = action_steps_per_chunk
        self.num_latent_per_block = num_latent_per_block
        ...
        # FIXME: if self.tag == EmbodimentTag.DREAM or self.tag == EmbodimentTag.LAPA:
        #     self.use_global_metadata = True
        self._lerobot_modality_meta = self._get_lerobot_modality_meta()
        logger.debug(f"Loaded modality metadata")
        self._lerobot_info_meta = self._get_lerobot_info_meta()
        logger.debug(f"Loaded info metadata")
        ...
        logger.debug(f"Loaded dataset statistics metadata")
        
        # Initialize trajectory info and chunk size early (needed for relative stats calculation)
        self._trajectory_ids, self._trajectory_lengths = self._get_trajectories()
        logger.debug(f"Loaded episode metadata for {len(self._trajectory_ids)} episodes from {self._dataset_path}")
        ...
        if self.relative_action and self._relative_action_keys_input is None:
            ...
            logger.debug(f"Relative action will be applied to keys: {self.relative_action_keys}")
        # Load relative action stats if relative_action is enabled
        logger.debug(f"relative_action={self.relative_action}")
        ...
        logger.debug(f"relative_action_per_horizon={self.relative_action_per_horizon}")
        ...
        logger.debug(f"loaded full dataset metadata")
        self._step_filter = self._get_step_filter()
        logger.debug(f"Loaded {len(self._step_filter)} step filter")
        self._all_steps = self._get_all_steps()
        logger.debug(f"Bootstrapped all steps with {len(self._all_steps)} total steps")

        # MUST run before _get_modality_keys(): that result is cached and is what
        # get_all_video_paths() iterates, so pruning after it would leave the
        # stale key list behind and the pack would still be asked for an mp4 dir
        # it does not have.
        #
        # An OPTIONAL goal-video track (the world-model track) is configured once
        # for the embodiment tag. Drop it when this pack does not declare it so a
        # single-pack or all-missing roster can fall back to the real future. A
        # same-tag mixture with only partial WM availability is still invalid:
        # metadata merging requires identical modalities, and launch preflight
        # rejects that heterogeneous case. Only wm_goal_video_key is pruned -- a
        # missing primary camera must remain a hard error.
        self._prune_absent_optional_video_keys()
        self._modality_keys = self._get_modality_keys()
        self._delta_indices = self._get_delta_indices()
        self._max_delta_index = self._get_max_delta_index()
        logger.debug(f"Max delta index: {self._max_delta_index}")
        self._dataset_name = self._dataset_path.name

        # NOTE(YL): method to predict the task progress
        if "action.task_progress" in self._modality_keys["action"]:
            from psi.data.gear.schema import StateActionMetadata

            logger.debug("we will add task progress to the action modality")
        ...
        logger.debug(f"Initialized dataset {self.dataset_name} with {embodiment_tag}")
    ...
    def _prune_absent_optional_video_keys(self) -> None:
        subgoal_cfg = self.modality_configs.get("subgoal")
        video_cfg = self.modality_configs.get("video")
        wm_key = getattr(subgoal_cfg, "kwargs", {}).get("wm_goal_video_key") if subgoal_cfg else None
        if not wm_key or video_cfg is None or wm_key not in video_cfg.modality_keys:
            return
        declared = set(self.lerobot_modality_meta.video or {})
        if wm_key.split(".", 1)[1] in declared:
            return
        # copy before mutating: the modality configs are shallow-copied per pack,
        # so editing the list in place would strip the key from every pack
        import copy as _copy
        pruned = _copy.deepcopy(video_cfg)
        pruned.modality_keys = [k for k in pruned.modality_keys if k != wm_key]
        self.modality_configs = dict(self.modality_configs)
        self.modality_configs["video"] = pruned
        logger.info(
            f"[goal sources] {self.dataset_name}: {wm_key} not declared in meta/modality.json; "
            f"pruning it from this pack's video keys -- its goal share falls back to the real future"
        )
    ...
    def _get_lerobot_stats_meta(self) -> dict[str, DatasetStatisticalValues]:
        ...
        try:
            ...
            logger.debug(f"Failed to load dataset statistics: {e}")
            logger.debug(f"Calculating dataset statistics for {self.dataset_name}")
    ...
    def _get_lerobot_relative_stats_meta(self) -> dict[str, DatasetStatisticalValues]:
        ...
        if stats_path.exists():
            logger.debug(f"Loading relative action stats from {stats_path}")
            with open(stats_path, "r") as f:
                stats: dict = json.load(f)
            for name, stat in stats.items():
                stats[name] = DatasetStatisticalValues.model_validate(stat)
        ...
        logger.debug(f"Relative stats file not found at {stats_path}")
        logger.debug(f"Calculating relative action stats for {self.dataset_name}")
        ...
        if not all_action_keys:
            logger.debug("No action keys found in modality configs, skipping relative stats calculation")
            return {}
        ...
        if not action_keys_to_process:
            logger.debug("No action keys to process for relative stats")
            return {}
        
        logger.debug(f"Will calculate relative stats for: {action_keys_to_process}")
        
        stats = {}
        for action_key in action_keys_to_process:
            logger.debug(f"Calculating relative stats for action key: {action_key}")
            try:
                relative_stats = self._calculate_relative_stats_for_key(action_key)
                stats[action_key] = relative_stats
            except Exception as e:
                logger.debug(f"Failed to calculate relative stats for {action_key}: {e}")
                continue
        ...
        if stats:
            ...
            logger.debug(f"Saving relative action stats to {save_path}")
    ...
    def _get_lerobot_relative_horizon_stats_meta(self) -> dict[str, dict[str, list]]:
        ...
        if stats_path.exists():
            logger.debug(f"Loading per-horizon relative action stats from {stats_path}")
            with open(stats_path, "r") as f:
                stats: dict = json.load(f)
            return stats
        
        # Calculate per-horizon relative stats if file doesn't exist
        logger.debug(f"Per-horizon relative stats file not found at {stats_path}")
        logger.debug(f"Calculating per-horizon relative action stats for {self.dataset_name}")
        
        # Get action keys from modality configs, filtered by relative_action_keys
        all_action_keys = self.modality_configs.get("action", ModalityConfig(delta_indices=[0], modality_keys=[])).modality_keys
        if not all_action_keys:
            logger.debug("No action keys found in modality configs, skipping per-horizon relative stats calculation")
            return {}
        ...
        if not action_keys_to_process:
            logger.debug("No action keys to process for per-horizon relative stats")
            return {}
        
        logger.debug(f"Will calculate per-horizon relative stats for: {action_keys_to_process}")
        
        stats = {}
        for action_key in action_keys_to_process:
            logger.debug(f"Calculating per-horizon relative stats for action key: {action_key}")
            try:
                relative_stats = self._calculate_relative_stats_for_key_per_horizon(action_key)
                stats[action_key] = relative_stats
            except Exception as e:
                logger.debug(f"Failed to calculate per-horizon relative stats for {action_key}: {e}")
                continue
        
        if stats:
            # Only save to dataset path (not global metadata path)
            save_path = self.dataset_path / LEROBOT_RELATIVE_HORIZON_STATS_FILE_NAME
            logger.debug(f"Saving per-horizon relative action stats to {save_path}")
            with open(save_path, "w") as f:
    ...
    def _calculate_relative_stats_for_key(self, action_key: str) -> DatasetStatisticalValues:
        ...
        logger.debug(f"Calculating relative stats for {action_key}:")
        logger.debug(f"  State: column='{state_original_key}', indices=[{state_start}:{state_end}]")
        logger.debug(f"  Action: column='{action_original_key}', indices=[{action_start}:{action_end}]")
        ...
        if len(traj_ids_to_process) > max_trajs_for_stats:
            ...
            logger.debug(f"Sampling {max_trajs_for_stats} trajectories out of {len(self.trajectory_ids)} for stats calculation")
        ...
        for traj_id in tqdm(traj_ids_to_process, desc=f"Calculating relative stats for {action_key}"):
            try:
                ...
                if state_original_key not in traj_data.columns or action_original_key not in traj_data.columns:
                    logger.debug(f"Missing columns: state='{state_original_key}' exists={state_original_key in traj_data.columns}, "
                ...
                for i in range(usable_length):
                    ...
                    # logger.debug("actions shape", actions.shape, "ref_state shape", ref_state.shape)
                ...
                logger.debug(f"Error processing trajectory {traj_id}: {e}")
                continue
        ...
        logger.debug(f"Collected {len(all_relative_actions)} relative action samples for {action_key}")
    ...
    def _calculate_relative_stats_for_key_per_horizon(
        ...
        logger.debug(f"Calculating per-horizon relative stats for {action_key}:")
        logger.debug(f"  State: column='{state_original_key}', indices=[{state_start}:{state_end}]")
        logger.debug(f"  Action: column='{action_original_key}', indices=[{action_start}:{action_end}]")
        logger.debug(f"  Action delta indices: {action_delta_indices}")
        ...
        for delta_idx in action_delta_indices:
            ...
            if not relative_actions:
                logger.debug(f"Warning: No relative actions calculated for {action_key} at horizon index {delta_idx}")
            ...
            logger.debug(f"Collected {len(relative_actions_array)} relative action samples for {action_key} at horizon {delta_idx}")
    ...
    def get_relative_stats_per_horizon(
        ...
        if action_keys is None:
            ...
            if not all_action_keys:
                logger.debug("No action keys found in modality configs")
                return {}
            # Default: apply to all action keys except those containing 'gripper'
            action_keys = [
        ...
        logger.debug(f"Calculating per-horizon relative stats for: {action_keys}")
        
        all_stats: dict[str, dict[str, list]] = {}
        
        for action_key in action_keys:
            logger.debug(f"Processing action key: {action_key}")
            try:
    ...
    def load_relative_stats_per_horizon(
        ...
        logger.debug(f"Loading per-horizon relative stats from {stats_path}")
        with open(stats_path, "r") as f:
    ...
    def _get_metadata(self) -> DatasetMetadata:
        ...
        for new_key in self.lerobot_modality_meta.video:
            ...
            # If `original_key` points at a parquet string column (per-frame
            # path interface; see get_video's path-column branch), the
            # feature dict has no "names"/"shape" / "video_info" fields. Use
            # the optional `image_info` sub-dict for the decoded-image dims.
            if le_video_meta.get("dtype") == "string" and "image_info" in le_video_meta:
                image_info = le_video_meta["image_info"]
                height = image_info["shape"][image_info["names"].index("height")]
                width = image_info["shape"][image_info["names"].index("width")]
                channels = image_info["shape"][image_info["names"].index("channel")]
                fps = image_info.get("fps", 30)
            else:
        ...
        # subgoal modality
        simplified_modality_meta["subgoal"] = {} #TODO
        ...
        for our_modality in ["state", "action"]:
            ...
            for subkey in simplified_modality_meta[our_modality]:
                ...
                if should_use_per_horizon:
                    ...
                    logger.debug(f"Using per-horizon relative stats for {subkey}")
                    ...
                    logger.debug(f"Using relative stats for {subkey}: {dataset_statistics[our_modality][subkey]}")
                else:
    ...
    def _get_trajectories(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        # Cache the full per-episode row dict for `meta.*` lookups in
        # `get_meta` (one value per episode, broadcast across step_indices
        # at retrieval time).
        self._episodes_meta: dict[int, dict] = {
            int(ep["episode_index"]): ep for ep in episode_metadata
        }
    ...
    def get_step_data(self, trajectory_id: int, indices: dict[str, np.ndarray], current_step: int | None = None) -> dict:
        ...
        Args:
            ...
            current_step (int | None): Accepted for API compatibility with the mixture's
                iterator (used by subclasses that sample subgoal frames); ignored here.
    ...
    def retrieve_data_and_pad(
        ...
        arr = step_indices.tolist()
        positions = np.arange(len(arr))
        if 0 in arr:
            last_zero_pos = len(arr) - 1 - arr[::-1].index(0)
            front_padding_indices = positions < last_zero_pos
        else:
            front_padding_indices = np.zeros(len(arr), dtype=bool)
        if (max_length - 1) in arr:
            first_maxlen_pos = arr.index(max_length - 1)
            end_padding_indices = positions > first_maxlen_pos
        else:
            end_padding_indices = np.zeros(len(arr), dtype=bool)
    ...
    def get_language(
        ...
        if subkey in METADATA_LANG_KEYS:
            # logger.debug("return metadata language")
        ...
        if pd.api.types.is_integer_dtype(col):
            # Stored as list of integers — compact task_index encoding
            task_indices: list[int] = col.iloc[step_indices].tolist()
            return self.tasks.loc[task_indices]["task"].tolist()
        elif pd.api.types.is_float_dtype(col):
            # Per-frame scalar annotation (e.g. sub_task_delta in seconds).
            # Pass the float values through; downstream decides how to use them.
            return col.iloc[step_indices].to_numpy(dtype=np.float32)
        else:
            # Stored as list of strings
            return col.iloc[step_indices].astype(str).tolist()
    ...
    def get_meta(
        ...
        """Look up a `meta.<key>` from modality.json against the cached
        per-episode row in `episodes.jsonl`.

        modality.json's `meta.<key>` is expected to be one of:
        - `{"original_key": "<episodes.jsonl field>"}` — pointer; the
          loader reads the trajectory's row and returns that field's
          value (broadcast to `len(step_indices)`).
        - a plain scalar (`str | int | float | bool | None`) — treated
          as a dataset-level constant, broadcast unchanged.

        Returns a list of length `len(step_indices)` so the value has
        the same time axis as other modalities even though it is
        per-episode invariant.
        """
        assert key.startswith("meta."), f"Meta key must start with 'meta.', got {key}"
        subkey = key.replace("meta.", "")
        meta_block = self.lerobot_modality_meta.meta
        assert meta_block is not None, f"Modality metadata has no `meta` block; got key={key}"
        assert subkey in meta_block, (
            f"Meta key {subkey} not found in metadata, available meta keys: "
            f"{list(meta_block.keys())}"
        )
        meta_field = meta_block[subkey]
        if isinstance(meta_field, dict) and "original_key" in meta_field:
            original_key = meta_field["original_key"]
            value = self._episodes_meta[int(trajectory_id)].get(original_key)
        else:
            value = meta_field
        return [value] * len(step_indices)
    ...
    def get_data_by_modality(
        ...
        # FIXME elif modality == "subgoal":
        #     return self.get_subgoal_image(trajectory_id, key, step_indices)
        else:
...
class CachedLeRobotSingleDataset(LeRobotSingleDataset):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for key in self.modality_keys["video"]:
            ...
            logger.debug(f"{key}: {cached_frames[key].shape}")
...
class LeRobotMixtureDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            allow_padding (bool): If True, allow padding at the beginning and end of the dataset.
        """
    ...
    def from_mixture_spec(
        ...
        if isinstance(dataset_class, str):
            ...
            module = importlib.import_module(module_name.replace("groot.", "dreamzero.groot."))
            ...
            for embodiment_tag, paths in dataset_spec.dataset_path.items():
                ...
                for dataset_path in paths:
                    ...
                    dataset = dataset_class(
                        ...
                        # set_metadata writes per-pack stats into the transforms; shallow copy
                        # would let train/val (or same-tag packs) clobber each other's stats
                        transforms=copy.deepcopy(all_transforms[embodiment_tag]),
    ...
    def merge_metadata(
        ...
        # Same-tag packs may expose different sub-key sets (e.g. extra eepose keys);
        # merge over the sub-keys common to all of them.
        def _common(section: str, modality: str) -> list[dict]:
            keys = set.intersection(*[set(m[section][modality]) for m in metadata_dicts])
            return [{k: m[section][modality][k] for k in keys} for m in metadata_dicts]

        # Merge the dataset statistics
        dataset_statistics = {}
        for modality in ("state", "action"):
            dataset_statistics[modality] = LeRobotMixtureDataset.compute_overall_statistics(
                per_task_stats=_common("statistics", modality),
        ...
        for modality in metadata_dicts[0]["modalities"]:
            configs = {json.dumps(c, sort_keys=True) for c in _common("modalities", modality)}
            assert (
                len(configs) == 1
    ...
    def update_metadata(self, metadata_config: dict) -> None:
        ...
        # Group metadata (and their sampling weights) by tag
        all_metadatas: dict[str, list[DatasetMetadata]] = {}
        all_weights: dict[str, list[float]] = {}
        for dataset, w in zip(self.datasets, self.dataset_sampling_weights.tolist()):
            all_metadatas.setdefault(dataset.tag.value, []).append(dataset.metadata)
            all_weights.setdefault(dataset.tag.value, []).append(w)
