...
from psi.data.gear.schema import DatasetMetadata, StateActionMetadata
...
class ConcatTransform(InvertibleModalityTransform):
    ...
    subgoal_concat_order: Optional[list[str]] = Field(
        default=None,
        description="Concatenation order for each subgoal modality. "
        "Format: ['subgoal.egocentric', ...].",
    )
    ...
    def apply(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key in data.keys():
            try:
                ...
                if "annotation" in key:
                    ...
                    # dotted sub-keys (e.g. action.wrists.left.xyz) -> real modality
        ...
        if "subgoal" in grouped_keys:
            subgoal_keys = grouped_keys["subgoal"]
            assert self.subgoal_concat_order is not None, f"{self.subgoal_concat_order=}, {subgoal_keys=}"
            assert all(
                item in subgoal_keys for item in self.subgoal_concat_order
            ), f"keys in subgoal_concat_order are misspecified, \n{subgoal_keys=}, \n{self.subgoal_concat_order=}"

            # Process each subgoal view
            unsqueezed_subgoals = []
            for subgoal_key in self.subgoal_concat_order:
                subgoal_data = data.pop(subgoal_key)
                # subgoal is dropout, skip concatenation
                if subgoal_data is None:
                    continue
                unsqueezed_subgoal = np.expand_dims(
                    subgoal_data, axis=-4
                )  # [..., H, W, C] -> [..., 1, H, W, C]
                unsqueezed_subgoals.append(unsqueezed_subgoal)

            if len(unsqueezed_subgoals) > 0:
                # Concatenate along the new axis
                unsqueezed_subgoal = np.concatenate(unsqueezed_subgoals, axis=-4)  # [..., V, H, W, C]
                data["subgoal"] = unsqueezed_subgoal
