...
from psi.data.gear.schema import DatasetMetadata, RotationType, StateActionMetadata
from psi.data.gear.transform.base import InvertibleModalityTransform, ModalityTransform
...
class StateActionTransform(InvertibleModalityTransform):
    ...
    def init_from_norm_stats(self, stats_for_tag: dict) -> None:
        """Initialize normalizers (and any missing modality_metadata) directly from a saved
        `{tag}_norm_stats.json` block: ``{modality: {subkey: {min,max,mean,std,q01,q99}}}``.

        Use when the transform is restored from a tyro/json config without building a dataset,
        so ``set_metadata`` was never called and ``_normalizers`` is empty. Assumes no rotation
        transforms (``target_rotations`` empty); ``modality_metadata.shape`` is taken from each
        key's stat dimension so ``denormalize_action``'s split/concat keeps working.
        """
        assert not self.target_rotations, (
            "init_from_norm_stats does not support rotations; use set_metadata(dataset_metadata)"
        )
        for key in self.apply_to:
            modality, subkey = key.split(".", 1)
            stats = stats_for_tag[modality][subkey]
            self.normalization_statistics[key] = stats
            # Rebuild minimal metadata only if it didn't survive config (de)serialization.
            if key not in self.modality_metadata:
                self.modality_metadata[key] = StateActionMetadata(
                    absolute=True, rotation_type=None, shape=(len(stats["min"]),), continuous=True,
                )
            if key in self.normalization_modes:
                self._normalizers[key] = Normalizer(
                    mode=self.normalization_modes[key], statistics=stats
                )
