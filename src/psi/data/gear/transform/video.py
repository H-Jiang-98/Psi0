...
from psi.data.gear.schema import DatasetMetadata
from psi.data.gear.transform.base import ModalityTransform
...
class VideoTransform(ModalityTransform):
    ...
    def set_metadata(self, dataset_metadata: DatasetMetadata):
        ...
        # No keys to resize (e.g. a config that lets the model do the resize):
        # treat as a no-op instead of asserting on an empty resolution set.
    ...
    def apply(self, data: dict[str, Any]) -> dict[str, Any]:
        if not self.apply_to:  # no-op when no keys are assigned
            return data
        if self.training:
...
class VideoToPIL(VideoTransform):
    ...
    def to_pil(frames: torch.Tensor) -> list:
        ...
        Args:
            frames: tensor of shape [T, C, H, W] in range [0, 1]
        Returns:
            list of PIL Images of length T
        """
        to_pil_fn = T.ToPILImage()
        return [to_pil_fn(frame) for frame in frames]
