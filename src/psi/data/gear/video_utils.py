...
def get_frames_by_timestamps(
    ...
    if video_backend == "decord":
        ...
        # print("!!! using torchcodec to decode video frames at timestamps")
        if not TORCHCODEC_AVAILABLE:
            raise ImportError("torchcodec is not available.")
        decoder = torchcodec.decoders.VideoDecoder(
        ...
        # cv2.VideoCapture's signature is (filename, apiPreference, params) — it does
        # NOT accept decord-style kwargs like num_threads. Translate num_threads to
        # cv2.setNumThreads (global thread control) and don't forward unknown kwargs.
        _ocv_kwargs = dict(video_backend_kwargs or {})
        _num_threads = _ocv_kwargs.pop("num_threads", None)
        if _num_threads is not None:
            cv2.setNumThreads(int(_num_threads))
        # Open the video file (ignore any remaining decord-only kwargs)
...
def get_frames_by_image_paths(
    dataset_root: Path,
    rel_paths: Sequence[str],
) -> np.ndarray:
    """Decode a sequence of JPEG paths (relative to `dataset_root`) into a
    contiguous (T, H, W, 3) uint8 image stack.

    Used for "video" modalities backed by per-frame image files rather than
    an mp4 stream (e.g. per-segment sub-goal images referenced from a
    parquet path-string column). Mirrors the (T, H, W, C) uint8 contract of
    `get_frames_by_timestamps` so callers can treat both backends uniformly.
    """
    dataset_root = Path(dataset_root)
    frames = [
        np.asarray(Image.open(dataset_root / rel).convert("RGB"))
