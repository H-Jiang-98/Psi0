...
class LeRobotModalityMetadata(BaseModel):
    ...
    meta: Optional[dict[str, Any]] = Field(
        default=None,
        description="Optional dataset-level metadata carried alongside the schema "
                    "(e.g. embodiment_tag, dataset-default quality/speed). The loader "
                    "does not consume this; it is informational and round-trips through "
                    "Pydantic for documentation purposes.",
    )
    subgoal: Optional[dict[str, LeRobotModalityField]] = Field(
        default=None,
        description="Optional subgoal image/video, To support subgoal steering in Psi-X"
    )
    ...
    def get_key_meta(self, key: str) -> LeRobotModalityField:
        ...
        if modality == "state":
            ...
            ), "Trying to get meta metadata for a dataset with no meta block"
            if subkey not in self.meta:
                raise ValueError(
                    f"Key: {key}, meta key {subkey} not found in metadata, available meta keys: {list(self.meta.keys())}"
                )
            # Wrap into a LeRobotModalityField so the return type matches
            # other modalities. Entry value can be either a dict with
            # `original_key` (per-episode pointer into episodes.jsonl) or
            # a plain scalar (dataset-level constant).
            entry = self.meta[subkey]
            if isinstance(entry, dict):
                return LeRobotModalityField.model_validate(entry)
            return LeRobotModalityField(original_key=None)
        elif modality == "subgoal":
            if subkey not in self.subgoal:
                raise ValueError(
                    f"Key: {key}, video key {subkey} not found in metadata, available video keys: {self.video.keys()}"
                )
            return self.subgoal[subkey]
        else:
